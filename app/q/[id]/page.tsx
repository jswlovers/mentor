...
export default function Detail() {
  ...
  if (data === undefined) return <p className="p-8 text-center text-sm text-muted">불러오는 중…</p>;
  if (data === null) return <p className="p-8 text-center text-sm text-muted">질문을 찾을 수 없어요.</p>;
    ...
    <div className="mx-auto max-w-2xl space-y-4 px-6 py-8">
      <div>
        <div className="mb-1 flex gap-1.5 text-xs">
          <span className="rounded bg-white/5 px-1.5 py-0.5 text-muted">{q.category}</span>
          <span className={q.status === "solved" ? "text-emerald-400" : "text-rose-400"}>{q.status === "solved" ? "해결됨" : "답변 대기"}</span>
        </div>
        ...
        <dl className="mt-3 rounded-lg border border-border bg-surface p-3 text-xs text-muted">
      ...
      {chatLabel && (
        <Link href={`/chat/${q.id}`} className="block rounded-lg border border-rose-500 py-2.5 text-center text-sm font-medium text-rose-400 hover:bg-rose-500/10">{chatLabel}</Link>
      )}
      <h2 className="text-sm font-semibold">답변 {answers.length}</h2>
      <ul className="space-y-3">
        {answers.map((a) => (
          <li key={a.id} className={`rounded-lg border p-3 ${a.accepted ? "border-emerald-500/40 bg-emerald-500/10" : "border-border bg-surface"}`}>
            <div className="mb-1 flex items-center gap-1.5 text-xs">
              {a.is_expert ? <Link href={`/experts/${a.author_id}`} className="font-bold underline">{a.author_name}</Link> : <b>{a.author_name}</b>}
              {a.is_expert ? <span className="rounded bg-rose-500/15 px-1.5 py-0.5 text-rose-300">검증 전문가</span> : null}
              {a.accepted ? <span className="text-emerald-400">채택됨</span> : null}
              <span className="text-muted">{timeAgo(a.created_at)}</span>
            </div>
            <p className="whitespace-pre-wrap text-sm">{a.body}</p>
            {isOwner && q.status === "open" && <button onClick={() => accept(a.id)} className="mt-2 text-xs text-emerald-400 underline">채택하기</button>}
          </li>
        ))}
      </ul>
      {me ? (
        <form onSubmit={answer} className="space-y-2">
          <textarea rows={3} maxLength={3000} className="w-full rounded-lg border border-border bg-surface-2 px-3 py-2 text-sm text-foreground placeholder:text-muted" placeholder="무료 답변 작성" value={text} onChange={(e) => setText(e.target.value)} />
          {err && <p className="text-xs text-rose-400">{err}</p>}
          <button className="w-full rounded-lg bg-rose-500 py-2.5 text-sm font-medium text-white hover:bg-rose-400">답변 등록</button>
        </form>
      ) : me === null ? (
        <p className="text-center text-sm text-muted"><Link href="/login" className="text-rose-400 underline">로그인</Link> 후 답변할 수 있어요.</p>
      ) : null}
    </div>
  );
}
