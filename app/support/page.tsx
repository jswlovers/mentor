  ...
  return (
    <div className="space-y-5 p-4">
      <form onSubmit={submit} className="space-y-2">
        ...
        <button className="w-full rounded-lg bg-neutral-900 py-2.5 text-sm font-medium text-white">접수</button>
        ...
        <ul className="space-y-2 text-sm">
          {tickets.map((t) => (
            <li key={t.id} className="rounded-lg border p-3">
              <p className="text-xs text-neutral-500">{CAT[t.category]} · {t.created_at} · {t.status === "pending" ? "접수됨" : "처리 완료"}</p>
              <p className="font-medium">{t.subject}</p>
              {t.refund_coins ? <p className="text-xs text-emerald-700">{t.refund_coins.toLocaleString()}코인 환불됨</p> : null}
              {t.admin_note && <p className="mt-1 text-xs text-neutral-600">답변: {t.admin_note}</p>}
            </li>
          ))}
          {tickets.length === 0 && <li className="text-neutral-400">접수한 문의가 없어요</li>}
        </ul>
      </section>
    </div>
  );
