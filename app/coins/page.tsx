  ...
  return (
    <div className="space-y-5 p-4">
      <section className="rounded-xl bg-rose-50 p-4">
        <p className="text-xs text-rose-700">내 코인 (1코인 = 1원)</p>
      ...
      <section className="rounded-xl border p-4 text-sm">
        ...
        <p className="mt-2 text-xs text-neutral-500">요금은 상담을 신청한 질문자에게만 부과되고, 전문가는 무료로 답변·통화해요. 전문가가 참여하기 전에는 전액 환불로 취소할 수 있어요.</p>
      </section>

      <section className="space-y-2">
        <h2 className="text-sm font-semibold">코인 충전 신청</h2>
        <select className="w-full rounded-lg border px-3 py-2 text-sm" value={amount} onChange={(e) => setAmount(Number(e.target.value))}>
          {[MIN_CHARGE_KRW, 10000, 50000, 100000, 300000].map((a) => <option key={a} value={a}>{a.toLocaleString()}원</option>)}
        </select>
        {info.tossClientKey && <button onClick={payToss} className="w-full rounded-lg bg-blue-600 py-2.5 text-sm font-medium text-white">카드로 바로 충전</button>}
        <div className="rounded-lg bg-neutral-50 p-3 text-sm">
          <p className="font-medium">무통장입금</p>
          <p className="mt-1">{info.bank ?? "입금 계좌가 아직 등록되지 않았어요. 고객센터로 문의해주세요."}</p>
          <input className="mt-2 w-full rounded-lg border bg-white px-3 py-2 text-sm" placeholder="입금자명 (입금 확인에 사용해요)" value={depositor} onChange={(e) => setDepositor(e.target.value)} maxLength={20} />
        </div>
        <button onClick={request} disabled={!info.bank} className="w-full rounded-lg bg-rose-600 py-2.5 text-sm font-medium text-white disabled:opacity-40">충전 신청 (무통장입금)</button>
        ...
        <ul className="divide-y text-sm">
          {charges.map((c) => (
            <li key={c.id} className="flex justify-between py-2">
              <span>{c.amount_krw.toLocaleString()}원 <span className="text-xs text-neutral-400">{c.provider === "toss" ? "카드" : "무통장"} · {c.created_at}</span></span>
              <span className={c.status === "approved" ? "text-emerald-600" : c.status === "rejected" ? "text-rose-600" : "text-amber-600"}>{STATUS[c.status] ?? c.status}</span>
            </li>
          ))}
          {charges.length === 0 && <li className="py-2 text-neutral-400">내역이 없어요</li>}
        </ul>
      </section>
        ...
        <ul className="divide-y text-sm">
          {ledger.map((l) => (
            <li key={l.id} className="flex justify-between gap-2 py-2">
              ...
              <span className={l.direction === "credit" ? "text-emerald-600" : "text-rose-600"}>{l.direction === "credit" ? "+" : "-"}{l.amount.toLocaleString()}</span>
            </li>
          ))}
          {ledger.length === 0 && <li className="py-2 text-neutral-400">내역이 없어요</li>}
        </ul>
      </section>
    </div>
  );
